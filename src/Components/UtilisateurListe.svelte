<script>
  ...
  function search() {
    ...
    for (const i of users) {
      if (i["identifiant"].toLowerCase().includes(recherche.toLowerCase())) {
        filtrer.push(i);
      }
    }
    totalPages = Math.ceil(filtrer.length / itemsPerPage);
...
{#if success}
  <div
    ...
    role="alert"
    style="position: fixed; bottom: 0; left: 20px;display:flex; gap:20px"
  >
    <svg
...
<table>
  ...
  <tbody>
    {#each actuel as user}
      <tr>
        <td data-label="Account">{user["identifiant"]}</td>
